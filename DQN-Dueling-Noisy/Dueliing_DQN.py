...
class NNDqn(nn.Module):
    ...
    def __init__(self, inSize, outSize,layers=[200], activation=torch.tanh,dropout=0.0):
         super(NNDqn, self).__init__()
         self.feature_layer=NN(inSize,layers[-1],layers=layers,activation=activation,dropout=dropout)
         self.advantage_layer=NN(layers[-1],outSize,layers=layers,activation=activation,dropout=dropout)
         self.value_layer=NN(layers[-1],1,layers=layers,activation=activation,dropout=dropout)
...
class DuelingDQN(object):
    def __init__(self, env,opt,layers=[200],activation=torch.tanh,droppout=0.0):
        ...
        self.optim=torch.optim.Adam(self.Q.parameters(),weight_decay=0.0,lr=opt.lr)

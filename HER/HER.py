...
if __name__ == '__main__':
    env, config, outdir, logger = init('./configs/config_random_gridworld.yaml', "HER")
    #------------------------------------------------------------------------------------------------
    freqTest = config["freqTest"]
    freqSave = config["freqSave"]
    nbTest = config["nbTest"]
    n_sampels = config["n_sampels"]
    ...
    for i in range(episode_count):
        ...
        while True:
            ...
            if j == config["maxLengthTrain"]:
                current_storage.append((ob, action, new_ob, goal, reward, False))
            else:
                current_storage.append((ob, action, new_ob, goal, reward, done))
            ...
            if done :
                ...
                rsum = 0
                #------------------------------------------------------------------
                for transition in current_storage:
                    agent.store(*transition)
                last_obs = current_storage[-1][2]

                for ob, action, new_ob, goal, reward, done in current_storage:
                    done_goal = (new_ob==last_obs).all()
                    reward_goal = 1.0 if done_goal else -0.1
                    agent.store(ob, action, new_ob, last_obs, reward_goal, done)
                #------------------------------------------------------------------
                break

...
class ValueIteration(RandomAgent):
    ...
    def learn_step(self):
        ...
        for s in self.mdp.keys():
            value_new[s]=np.max([np.sum([proba * (reward+self.gamma*(1-done)*self.value[next_state])  for proba,next_state,reward,done in  self.mdp[s].get(action)]) for action in range(self.n_action)])
        self.value=copy.deepcopy(value_new) 
        for s in self.mdp.keys():
            self.policy[s]=np.argmax([np.sum([proba * (reward+self.gamma*(1-done)*self.value[next_state])  for proba,next_state,reward,done in self.mdp[s].get(action)]) for action in range(self.n_action)])
